...
2. **Create the change directory**
   ```bash
   aegis new change "<name>"
   ```
   This creates a scaffolded change at `aegis/changes/<name>/` with `.aegis.yaml`.
...
4. **Create artifacts in sequence until apply-ready**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      - Get instructions:
        ```bash
        aegis instructions <artifact-id> --change "<name>" --json
        ```
   ...
   b. **Continue until all `applyRequires` artifacts are complete**
      - After creating each artifact, re-run `aegis status --change "<name>" --json`
...
- Follow the `instruction` field from `aegis instructions` for each artifact type
